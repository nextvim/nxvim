    ...
    pub fn execute(
        ...
        if WindowOps::window_buffer(ui, active_window) == Some(model.commandline_buffer()) {
            if model.commandline_mode == '/' || model.commandline_mode == '?' {
                if let Some(window) = ui.window(active_window).and_then(vim_ui::Window::window_state) {
                    if let Ok(buffer) = model.get_buffer(model.commandline_buffer()) {
                        if let Some(selection) = window.selections.first() {
                            ...
                            let end = Point::new(current_row, text_buffer.line_len(current_row)).to_offset(text_buffer);
                            let pattern: String = text_buffer.as_rope().chunks_in_range(start..end).collect();
                            if pattern.is_empty() {
