...
impl Ui {
    ...
    pub fn window(&self, id: usize) -> Option<&window::Window> {
        self.windows.get(&id)
    }

    pub fn window_mut(&mut self, id: usize) -> Option<&mut window::Window> {
    ...
    pub fn cancel_document_parse_tasks(&mut self) {
        for window in self.windows.values_mut() {
            if let Some(document) = window.doc.as_mut() {
                document
    ...
    fn layout(&mut self, screen_cols: u32, screen_rows: u32) -> bool {
        ...
        if self.screen_cols == screen_cols
            && self.screen_rows == screen_rows
            && self.last_parent_rect.is_some()
        {
            return false;
        }
        self.screen_rows = screen_rows;
    ...
    pub fn split_focused_window(
        ...
        self.editor_layout
            .split_leaf(focused_id, new_win_id, direction);
    ...
    pub fn only_windows(&mut self) {
        ...
        self.editor_layout = layout::LayoutNode::Leaf {
    ...
    pub fn adjust_focused_window_size(&mut self, direction: layout::SplitDirection, amount: f32) {
        ...
        if self
            .editor_layout
            .adjust_size(focused_id, direction, amount)
        {
        ...
        for popup in &mut self.popup_stack {
            unsafe {
                if popup.is_visible() {
                    popup
                        .window
                        .draw(stdout, popup.rect, editor, buffer_manager, &*ui_ptr)?;
                }
            }
        }
...
mod tests {
    ...
    fn test_create_popup() {
        ...
        assert_eq!(ui.popup_stack.len(), 1);
        assert_eq!(ui.popup_stack[0].window.id, popup_id);
