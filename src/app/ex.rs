...
use crate::kernel::{CommandKind, CommandLineRequest, CommandMetadata, EditorContext, ExAdmission};

fn command_metadata(command: &Command) -> CommandMetadata {
    let (kind, count, range, register) = match command {
        Command::Editor { action, register } => {
            ...
            (kind, Some(action.count() as usize), None, *register)
        }
        Command::RangeOp {
            range,
            count,
            ...
            CommandKind::Edit,
            count.map(|value| value as usize),
            range.clone(),
            *register,
        ),
        Command::Substitute { range, .. } => (CommandKind::Edit, None, range.clone(), None),
        Command::SearchForward { .. } | Command::SearchBackward { .. } => {
            (CommandKind::Edit, None, None, None)
        }
        Command::Save { .. }
        | Command::Edit { .. }
        | Command::WriteQuit { .. }
        ...
        | Command::Indexer { .. }
        | Command::Inspect { .. }
        | Command::Echo { .. } => (CommandKind::Ex, None, None, None),
        Command::SplitNew { .. } | Command::BufferNext { .. } | Command::BufferPrevious { .. } => {
            (CommandKind::Window, None, None, None)
        }
        Command::TabNew { .. }
        | Command::TabNext { .. }
        | Command::TabPrevious { .. }
        | Command::TabClose => (CommandKind::Tab, None, None, None),
        Command::Set { .. } | Command::SetOption { .. } => (CommandKind::Option, None, None, None),
        Command::ReplaceBuffer { .. } => (CommandKind::Edit, None, None, None),
        Command::ExecuteScript(_) | Command::OpenPrompt { .. } | Command::Task(_) => {
            (CommandKind::Script, None, None, None)
        }
    };
    CommandMetadata {
        kind,
...
/// Application orchestration for parsed Ex and script-host commands.
///
/// Stable-context admission belongs to the semantic kernel. This adapter owns
/// lifecycle, services, UI projection, and other application-level effects.
...
impl ExDispatcher {
    ...
    /// Executes a command emitted by the script host after kernel admission.
    pub fn execute_host_command(
        ...
        let Some(context) = app
            .model
            .kernel()
            .command_context_with(command_metadata(&command))
        else {
...
mod tests {
    ...
    #[test]
    fn compatibility_commands_translate_to_neutral_kernel_metadata() {
        let motion = command_metadata(&Command::Editor {
            action: vim_input::Action::MoveLeft {
                count: 3,
                select: false,
            },
            register: Some('a'),
        });
        assert_eq!(motion.kind, CommandKind::Motion);
        assert_eq!(motion.count, Some(3));
        assert_eq!(motion.register, Some('a'));
        ...
        let ranged = command_metadata(&Command::RangeOp {
            operation: crate::app::range_ops::RangeOperation::Delete,
            bang: false,
            range: Some(range.clone()),
            count: Some(2),
            register: Some('b'),
        });
        assert_eq!(ranged.kind, CommandKind::Edit);
        assert_eq!(ranged.count, Some(2));
        assert_eq!(ranged.range, Some(range));
        assert_eq!(ranged.register, Some('b'));
    }
