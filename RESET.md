...
- [ ] **Connect script autocommands**
  - [ ] **Freeze the event bridge and callback snapshot**
    - [ ] Translate each supported `kernel::EditorEvent` to a canonical Vim event name, match subject, owned payload, and originating `EditorContext` when one exists.
    - [ ] Snapshot matching handler IDs/actions in registration order before executing any callback; callback-side buffer renames, deletion, group clearing, or new registrations must not alter the current dispatch set.
    - [ ] Populate owned autocmd context for `<amatch>`, `<afile>`, `<abuf>`, and `v:event`; do not retain buffer/window references across callback execution.
  - [ ] **Complete registration and removal semantics**
    - [ ] Preserve additive `:autocmd` registration order and comma-separated event/pattern expansion.
    - [ ] Support explicit `[group]`, current `:augroup`, `augroup END`, group existence checks, and group-scoped clearing.
    - [ ] Implement selective `:autocmd! [group] {event} {pattern}` removal instead of clearing an entire group or the whole bus whenever bang is present.
  ...
  - [ ] **Implement Vim-compatible event matching**
    - [ ] For file events, match patterns without `/` against the filename tail; match patterns containing `/` against both the entered short name and canonical full path.
    - [ ] Expand environment variables and `~` at definition time and support escaped commas, character classes, alternation braces, and escaped wildcard literals.
    - [ ] Use event-specific subjects for non-file events, including long option names for `OptionSet`; do not assume every event matches a path.
    - [ ] Keep event names canonical/case-sensitive and reject unknown names and `*` during registration.
  - [ ] **Add suppression, once, and nesting policy**
    - [ ] Consume `++once` handlers when selected for execution so recursive or failing callbacks cannot schedule them twice.
    - [ ] Suppress events generated by a non-`++nested` callback; permit nested dispatch only from `++nested` handlers and cap recursion at Vim's 10-level limit.
    - [ ] Apply `:noautocmd` to the complete controlled host command and implement global `eventignore` filtering, including `all` and exclusions such as `all,-WinLeave`.
    - [ ] Prevent recursive `OptionSet` delivery while an `OptionSet` callback changes an option, matching Vim's documented non-recursive behavior.
  - [ ] **Execute through controlled host requests**
    - [ ] Add a script-runtime event entry point that accepts an owned event envelope and returns/enqueues callbacks with the event's originating stable tab/window/buffer IDs.
    - [ ] Route callback-produced editor commands through `ExDispatcher::execute_host_command`; do not call controller compatibility dispatch directly.
    - [ ] Preserve callback order across immediate events, deferred event batches, and commands emitted asynchronously by the script scheduler.
    - [ ] Define error behavior: report a callback error, stop the remaining commands for that handler where Vim does, and continue later handlers only where Vim semantics permit.
  - [ ] **Revalidate identity after every callback command**
    - [ ] Validate the originating tab/window/buffer before each controlled host request, not only once before the handler starts.
    - [ ] After nested callbacks, reject stale identities and refresh current context only through explicit event semantics; never silently retarget a stale callback to the new current buffer/window.
    - [ ] Ensure destructive events can expose `<abuf>`/`<afile>` without requiring the deleted object to remain live.
  - [ ] **Focused autocommand gate**
    - [ ] Add parser/registry tests for augroups, selective clearing, duplicate order, `<buffer>`, pattern rules, and `++once`.
    - [ ] Add dispatch tests for non-nested suppression, `++nested`, the depth limit, `eventignore`, `:noautocmd`, callback errors, and self-modifying registrations.
    ...
    - [ ] Run `cargo test -p vim-script`, focused `nxvim` event tests, `cargo check -p vim-script`, and `cargo check -p nxvim`.
...
- Nest suppression, the 10-level recursion limit, `eventignore`, `:noautocmd`, autocmd special variables, and stale-identity behavior are not implemented yet.
