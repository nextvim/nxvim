...
`nxvim` is a Rust workspace for Vim-compatible editing components. Its first reusable component is the internal `crates/vim-buffer` library, which provides text buffers, lifecycle management, selections, transactions, and mutation semantics using Zed's Rope + SumTree-backed `text::Buffer` rather than Vim's `memline`/`memfile` internals.

The compatibility target is **observable Vim behavior**, not Vim's internal representation. `vim-buffer` is editor agnostic: it does not depend on `nxvim` UI/application code, `dzed`, rendering, terminal input, syntax services, or an async runtime. It is intended to support `vim-script`, `vim-regex`, `vim-formatter`, `dzed`, and other consumers independently, and may later be promoted into its own repository/library without redesigning its API.
...
The compatibility oracle is pinned to upstream Vim **9.2, patches 1–843** (`v9.2.0843`), commit `975e191dc817d8d00abca7197c4529a417c2f805`. The machine-readable contract and reproducible build settings are in `oracle/vim-version.json` and `oracle/README.md`. This is the same explicit oracle version used by `nextvim/vim-regex`; pin changes must be coordinated across the `nextvim` libraries.
...
`crates/zed` is a generated, nested Cargo workspace extracted from current upstream Zed rather than copied from `dzed`'s older snapshot. The inspected `dzed` tree contains no intentional patches to the four target crates; its archived Zed version is older, particularly in `text`, `rope`, and `sum_tree`.
...
`vim_buffer::Transaction` is a Vim validation/planning facade over Zed transactions. Zed remains the unit that applies text atomically and owns primitive undo history; the facade defines Vim command boundaries, `changedtick`, selection mapping, outcomes, and callback classification.

```rust
let snapshot = buffer.snapshot();
let mut tx = buffer.transaction(EditOrigin::User, &snapshot);
tx.replace_for(selection_id, range, text)?;
tx.insert_for(other_selection_id, offset, more_text)?;
let outcome = tx.commit_with_selections(selection_set)?;
```
...
Buffer and manager mutation is single-threaded and synchronous. `nxvim` does not put `Buffer` or `BufferManager` behind internal locks, does not provide concurrent mutation, and does not depend on an async runtime. Mutation APIs require exclusive access on the owning editor thread.
...
The public contract should require snapshots used by background services to be `Send + Sync`. Add a compile-time assertion test for this property so changes in the extracted Zed crate cannot silently break the boundary.
...
Unlike the current `dzed` prototype, the mutator must not call `buffer.edit` separately for each cursor. It submits one normalized transaction and receives one outcome. Mode changes, operator composition, clipboard/register requests, and follow-up actions remain executor concerns; text, revision, marks, and undo remain buffer concerns.
...
Start with one editor-agnostic internal library. Keep its dependencies pointing downward to the extracted storage crates so it can later move to a standalone repository without pulling the `nxvim` application with it:

```text
nxvim workspace
├── crates/
│   ├── vim-buffer/             # first-party, editor-agnostic library
│   │   ├── Cargo.toml
│   │   └── src/
│   │       ├── lib.rs
│   │       ├── buffer.rs
...
│   │       ├── edit.rs
│   │       ├── transaction.rs
│   │       ├── mutator.rs      # synchronous Action execution/callback ordering
│   │       ├── callback.rs     # VimEvent registry/read-only contexts
│   │       ├── outcome.rs      # mutation and lifecycle results
│   │       ├── position.rs
│   │       ├── snapshot.rs
│   │       ├── error.rs
│   │       ├── options.rs
...
│   │       ├── undo.rs
│   │       ├── history.rs
│   │       ├── selection.rs    # Vim wrapper around Zed Selection<Anchor>
│   │       ├── selection_set.rs
│   │       └── io.rs
│   └── zed/                    # generated third-party nested workspace
│       └── crates/{clock,rope,sum_tree,text,...}
└── src/                        # nxvim application/integration only
```

`vim-buffer` depends directly on `text` and on `clock` only for Zed's public version-vector type. Rope and SumTree remain transitive implementation details of `text`; `vim-buffer` must not build a parallel storage layer from them. The root application may depend on `vim-buffer`, but `vim-buffer` must never depend on the root package or an editor frontend. Public APIs must not mention display maps, windows, terminal events, rendering types, tree-sitter, diagnostics, or application service containers.

Optional features/modules after the core stabilizes:

```text
search.rs          snapshot traversal APIs; no dependency on vim-regex
extmark.rs         generalized anchored metadata
serialization.rs   library-owned persistent state
```

Diagnostics, syntax caches, tree-sitter caches, and registers remain separate crates or consumer-owned layers unless a future architecture revision establishes a core ownership requirement.

### Initial scaffold status

The module and type skeleton now exists under `crates/vim-buffer/src`:
...
Placeholder methods fail explicitly with `BufferError::NotImplemented`; they must never report success before preserving the documented invariants. The scaffold is an API/ownership checkpoint, not a claim that Vim-compatible mutation is implemented.

## Public API direction

The library should expose traits where consumers need abstraction, but avoid trait-heavy internals prematurely.
...
Regex and formatter integrations should consume chunk iterators/snapshots rather than flattening the rope or owning buffer text. Vimscript integration should use `BufferId` and checked manager/buffer methods rather than direct field access.
...
**Exit criteria:** each target consumer can read and mutate through public APIs without accessing rope internals or flattening full buffers.
...
- Use the revision-pinned upstream Zed extraction under `crates/zed`, accessed through `nxvim` APIs rather than depending on the `dzed` application.
...
The initial foundation decisions are resolved. Remaining details—individual callback payloads, exact Vim event ordering, and commands allowed to queue from callbacks—must be specified incrementally from `autocmd.txt` and differential tests before each related API is stabilized.
